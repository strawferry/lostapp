...
const ajax_tools_space = {
  ...
  getMatchedInterface: ({ thisRequestUrl = '', thisMethod = '' }) => {
    ...
    return interfaceList.find(({ open = true, matchType = 'normal', matchMethod, request }) => {
      ...
      // if (thisRequestUrl.includes('api.macklin.cn/api')) {
      //   return true;
      // }
  ...
  myXHR: function () {
    const modifyResponse = () => {
      ...
      if (requestUrl.includes('api.macklin.cn/api')) {
    ...
    for (const attr in xhr) {
      if (attr === 'onreadystatechange') {
        ...
        this.open = (...args) => {
          ...
          if (!requestUrl.includes('/api/product/info') && !requestUrl.includes('/api/timestamp')) {
            console.log("🚀 ~ 取消请求 requestUrl:", requestUrl)
