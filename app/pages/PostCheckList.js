...
class PostCheckList extends Component {
    // 构造
    constructor(props) {
        super(props);
        // 初始状态
    ...
    componentDidMount() {
        this.props.MeAction.postcheck(false, null, null, this.state.type, this.props.user.token);
    }
    ...
    _handleInit = ()=>{
        const { MeAction } = this.props;
            MeAction.postcheck(true, null, null, this.state.type, this.props.user.token);
    };
    _handleNew = ()=>{
        const { MeAction, data } = this.props;
            if(data.length>0){
                MeAction.postcheck(false, data[0]._id , "new", this.state.type, this.props.user.token);
            }else{
                MeAction.postcheck(false, null, "no data", this.state.type, this.props.user.token);
            }
    };
    _handleMore = ()=>{
        const { MeAction, data } = this.props;
            if(data.length>0){
                MeAction.postcheck(false, data[data.length-1]._id, "more", this.state.type, this.props.user.token);
            }else{
                MeAction.postcheck(false, null, "no data", this.state.type, this.props.user.token);
            }
...
import { postcheck, clearData } from './../actions/MeAction';
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        MeAction: bindActionCreators({ postcheck, clearData}, dispatch),
        PostAction: bindActionCreators({setDetailID, setDetailData}, dispatch),
    }
};
export default connect(mapStateToProps, mapDispatchToProps)(PostCheckList);