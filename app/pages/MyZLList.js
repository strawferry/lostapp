...
class MyZLList extends Component {
    // 构造
    constructor(props) {
        super(props);
        // 初始状态
    ...
    componentDidMount() {
        this.props.MeAction.myzllist(null, null, null, null, this.props.user.token)
    }
    ...
    _handleInit = ()=>{
        ...
        if (this.state.type === 0){
            MeAction.myzllist(true, null, null, null, this.props.user.token);
        }else if (this.state.type === 1){
            console.log('---------------1-------');
            MeAction.myzllist(true, 0, null, null, this.props.user.token);
        }else if (this.state.type === 2){
            MeAction.myzllist(true, 1, null, null, this.props.user.token);
        }
    };
    _handleNew = ()=>{
        ...
        if (this.state.type === 0){
            if(data.length>0){
                MeAction.myzllist(false, null, data[0]._id , "new", this.props.user.token);
            }else{
                MeAction.myzllist(false, null, null, "no data", this.props.user.token);
            }
        }else if (this.state.type === 1){
            if(data.length>0){
                MeAction.myzllist(false, 0, data[0]._id , "new", this.props.user.token);
            }else{
                MeAction.myzllist(false, 0, null, "no data", this.props.user.token);
            }
        }else if (this.state.type === 2){
            if(data.length>0){
                MeAction.myzllist(false, 1, data[0]._id , "new", this.props.user.token);
            }else{
                MeAction.myzllist(false, 1, null, "no data", this.props.user.token);
            }
        }
    };
    _handleMore = ()=>{
        ...
        if (this.state.type === 0){
            if(data.length>0){
                MeAction.myzllist(false, null, data[data.length-1]._id, "more", this.props.user.token);
            }else{
                MeAction.myzllist(false, null, null, "no data", this.props.user.token);
            }
        }else if (this.state.type === 1){
            if(data.length>0){
                MeAction.myzllist(false, 0, data[data.length-1]._id, "more", this.props.user.token);
            }else{
                MeAction.myzllist(false, 0, null, "no data");
            }
        }else if (this.state.type === 2){
            if(data.length>0){
                MeAction.myzllist(false, 1, data[data.length-1]._id, "more", this.props.user.token);
            }else{
                MeAction.myzllist(false, 1, null, "no data", this.props.user.token);
            }
        }
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        MeAction: bindActionCreators({ myzllist, clearData}, dispatch),
        PostAction: bindActionCreators({setDetailID, setDetailData}, dispatch),
    }
};
export default connect(mapStateToProps, mapDispatchToProps)(MyZLList);